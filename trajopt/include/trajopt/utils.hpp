...
inline DblVec trajToDblVec(const int trajectory_index, const TrajArray& x)
{
    const auto start_offset = x.cols() * trajectory_index;
    const auto end_offset = x.cols() * (trajectory_index + 1);
    return DblVec(x.data() + start_offset, x.data() + end_offset);
}
